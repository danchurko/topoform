...
const parallelogramRatio=208/108,parallelogramSlant=2*Math.tan(Math.PI/12)/parallelogramRatio,parallelogramPoints=[-1,-1,1-parallelogramSlant,-1,1,1,-1+parallelogramSlant,1];
const nodes=model.nodes.map(n=>{
  ...
  const labelWidth=appearance.shape==='diamond'?96:appearance.shape==='parallelogram'?112:appearance.shape==='oval'?116:appearance.shape==='square'?104:152,displayLabel=lines(n.label,labelWidth).join('\n');
  const labelHeight=displayLabel.split('\n').length*19,contentHeight=labelHeight+(n.icon?30:0)+24;
  let width=group?180:180,height=Math.max(80,contentHeight);
  if(appearance.shape==='square')width=height=Math.max(116,height);
  if(appearance.shape==='diamond')width=height=Math.max(160,Math.ceil(labelWidth+labelHeight+20));
  if(appearance.shape==='oval'){width=Math.max(200,width);height=Math.max(112,contentHeight+18);}
  if(appearance.shape==='parallelogram'){height=Math.max(108,contentHeight+16);width=height*parallelogramRatio;}
  const slant=appearance.shape==='parallelogram'?parallelogramSlant:0;
  ...
  return {data:{id:n.id,label:n.label,displayLabel,kind:n.kind||'element',...(n.parent?{parent:n.parent}:{}),width,height,labelWidth,labelHeight,shape,slant,icon:icons[n.icon]||null,iconTop,fill:appearance.fill||'#ffffff',stroke:appearance.stroke||colors.get(n.kind||'element')||'#7c8899',strokeWidth:appearance.strokeWidth||1.7,textColor:appearance.textColor||'#203047',groupLabel:appearance.groupLabel||'top',groupPadding:appearance.groupLabel==='inside'?56:48,groupTextX:appearance.groupLabel==='inside'?(n.icon?42:14)+groupLabelWidth:0}};
});
const nodeSpacing=Math.max(90,model.layout?.spacing||0),layerSpacing=Math.max(180,model.layout?.layerSpacing||0);
...
const cy=api.cy=cytoscape({
  ...
  style:[
    ...
    {selector:'node:parent',style:{'shape':'round-rectangle','background-color':'data(fill)','background-opacity':0.22,'border-color':'data(stroke)','border-width':'data(strokeWidth)','border-style':'dashed','padding':'data(groupPadding)','label':'data(label)','font-size':12,'font-weight':600,'color':'data(textColor)','text-valign':'top','text-halign':'center','text-margin-x':0,'text-margin-y':-9,'text-background-color':'#fcfcfd','text-background-opacity':1,'text-background-padding':3,'text-background-shape':'roundrectangle','compound-sizing-wrt-labels':'exclude','background-image':'none','text-wrap':'wrap','text-max-width':300}},
...
cy.renderer().nodeShapes.rhomboid=cy.renderer().nodeShapes.makePolygon(parallelogramPoints);
...
function portPoint(node,side,t){
  const w=node.data('width'),h=node.data('height'),shape=node.data('shape'),slant=node.data('slant'),horizontal=side==='NORTH'||side==='SOUTH';let x=horizontal?t:0,y=horizontal?0:t;
  ...
  else if(shape==='rhomboid'){if(horizontal){x=(side==='SOUTH'?slant/2:-slant/2)+(1-slant/2)*t;y=side==='SOUTH'?1:-1;}else x=(side==='EAST'?1-slant/2:-1+slant/2)+slant*y/2;}
...
function crossesNode(a,b,node){
  ...
  if(shape==='rhomboid'){const s=node.data('slant');if(Math.abs(ay-by)<.001){if(Math.abs(ay)>=m)return false;const shift=s*ay/2;return Math.max(Math.min(ax,bx),-m+s*m/2+shift)<Math.min(Math.max(ax,bx),m-s*m/2+shift);}const low=Math.max(-m,2*ax/s-2*m/s+m),high=Math.min(m,2*ax/s+2*m/s-m);return Math.max(Math.min(ay,by),low)<Math.min(Math.max(ay,by),high);}
  return Math.abs(x)<m&&Math.abs(y)<m;
}
async function arrange(elements){
  const elk={'elk.algorithm':'layered','elk.direction':$('direction').value,'elk.hierarchyHandling':'INCLUDE_CHILDREN','elk.edgeRouting':'ORTHOGONAL','elk.spacing.nodeNode':nodeSpacing,'elk.spacing.edgeNode':Math.max(32,nodeSpacing/2),'elk.spacing.edgeEdge':24,'elk.layered.spacing.nodeNodeBetweenLayers':layerSpacing,'elk.layered.spacing.edgeNodeBetweenLayers':Math.max(36,layerSpacing/3),'elk.layered.spacing.edgeEdgeBetweenLayers':24,'elk.padding':'[top=40,left=40,bottom=40,right=40]','elk.randomSeed':17};
...
async function route(elements,retryEdge=null){
  ...
  visible.forEach(edge=>{if(edge.isLoop()){edge.data({curveStyle:'bezier',routeLabelX:0,routeLabelY:0});return;}const source=edge.source().position(),target=edge.target().position(),sourceSide=side(source,target),targetSide=opposite[sourceSide],record={edge,source,target,sourceSide,targetSide};records.push(record);for(const [node,which,s,peer] of [[edge.source(),'source',sourceSide,target],[edge.target(),'target',targetSide,source]]){const key=`${node.id()}:${s}`;if(!ports.has(key))ports.set(key,[]);ports.get(key).push({record,which,peer});}});
  for(const entries of ports.values()){const vertical=['EAST','WEST'].includes(entries[0].record[`${entries[0].which}Side`]);entries.sort((a,b)=>(vertical?a.peer.y-b.peer.y:a.peer.x-b.peer.x)||a.record.edge.id().localeCompare(b.record.edge.id()));entries.forEach((entry,i)=>{entry.record[`${entry.which}Port`]=entries.length===1?0:-.8+1.6*i/(entries.length-1);entry.record[`${entry.which}Stub`]=28+i*16;});}
  const obstacles=elements.nodes(':visible').filter(n=>!n.isParent()).map(n=>{const p=n.position(),pad=12;return{id:n.id(),x1:p.x-n.data('width')/2-pad,y1:p.y-n.data('height')/2-pad,x2:p.x+n.data('width')/2+pad,y2:p.y+n.data('height')/2+pad};}),groupClearance=22,groups=elements.nodes(':visible').filter(n=>n.isParent()).map(n=>{const b=n.boundingBox({includeLabels:false,includeOverlays:false});return{id:n.id(),x1:b.x1,y1:b.y1,x2:b.x2,y2:b.y2};}),xs=[...obstacles.flatMap(o=>[o.x1-18,o.x2+18]),...groups.flatMap(g=>[g.x1-groupClearance,g.x2+groupClearance])],ys=[...obstacles.flatMap(o=>[o.y1-18,o.y2+18]),...groups.flatMap(g=>[g.y1-groupClearance,g.y2+groupClearance])];
  const segments=path=>path.slice(1).map((p,i)=>[path[i],p]),intersects=(a,b,o)=>Math.abs(a.x-b.x)>=Math.abs(a.y-b.y)?a.y>o.y1&&a.y<o.y2&&Math.max(a.x,b.x)>o.x1&&Math.min(a.x,b.x)<o.x2:a.x>o.x1&&a.x<o.x2&&Math.max(a.y,b.y)>o.y1&&Math.min(a.y,b.y)<o.y2;
  const hugsGroup=(a,b,g)=>Math.abs(a.y-b.y)<.5?Math.max(a.x,b.x)>g.x1&&Math.min(a.x,b.x)<g.x2&&Math.min(Math.abs(a.y-g.y1),Math.abs(a.y-g.y2))<groupClearance:Math.max(a.y,b.y)>g.y1&&Math.min(a.y,b.y)<g.y2&&Math.min(Math.abs(a.x-g.x1),Math.abs(a.x-g.x2))<groupClearance;
  const insideGroup=(path,g)=>path.every(p=>p.x>=g.x1+groupClearance&&p.x<=g.x2-groupClearance&&p.y>=g.y1+groupClearance&&p.y<=g.y2-groupClearance);
  const facingSide=(g,p)=>{const dx=p.x-(g.x1+g.x2)/2,dy=p.y-(g.y1+g.y2)/2;return Math.abs(dx)>=Math.abs(dy)?(dx<0?'WEST':'EAST'):(dy<0?'NORTH':'SOUTH');};
  const leavesOnce=(path,g,fromSource)=>{const points=fromSource?path:[...path].reverse(),inside=p=>p.x>=g.x1&&p.x<=g.x2&&p.y>=g.y1&&p.y<=g.y2,i=points.findIndex(p=>!inside(p));return i>0&&!points.slice(i).some(inside)&&!segments(points.slice(i)).some(([a,b])=>intersects(a,b,g));};
  const leavesThrough=(path,g,fromSource,side)=>{const points=fromSource?path:[...path].reverse(),i=points.findIndex(p=>p.x<g.x1||p.x>g.x2||p.y<g.y1||p.y>g.y2);if(!leavesOnce(path,g,fromSource))return false;const a=points[i-1],b=points[i],horizontal=Math.abs(a.y-b.y)<.5;if(side==='WEST')return horizontal&&b.x<g.x1;if(side==='EAST')return horizontal&&b.x>g.x2;if(side==='NORTH')return!horizontal&&b.y<g.y1;return!horizontal&&b.y>g.y2;};
  const parallelGap=(a,b,c,d)=>{const ah=Math.abs(a.y-b.y)<.5,ch=Math.abs(c.y-d.y)<.5;if(ah!==ch)return Infinity;const shared=ah?Math.min(Math.max(a.x,b.x),Math.max(c.x,d.x))-Math.max(Math.min(a.x,b.x),Math.min(c.x,d.x)):Math.min(Math.max(a.y,b.y),Math.max(c.y,d.y))-Math.max(Math.min(a.y,b.y),Math.min(c.y,d.y));return shared>3?Math.abs(ah?a.y-c.y:a.x-c.x):Infinity;};
  ...
  const compact=path=>{const out=[];for(const p of path){const n=out.length,vertical=n>1&&Math.abs(out[n-2].x-out[n-1].x)<.01&&Math.abs(out[n-1].x-p.x)<.01&&(out[n-1].y-out[n-2].y)*(p.y-out[n-1].y)>=0,horizontal=n>1&&Math.abs(out[n-2].y-out[n-1].y)<.01&&Math.abs(out[n-1].y-p.y)<.01&&(out[n-1].x-out[n-2].x)*(p.x-out[n-1].x)>=0;if(vertical||horizontal)out[n-1]=p;else if(!n||Math.abs(p.x-out[n-1].x)>.01||Math.abs(p.y-out[n-1].y)>.01)out.push(p);}return out;};
  const area=(a,b)=>Math.max(0,Math.min(a.x2,b.x2)-Math.max(a.x1,b.x1))*Math.max(0,Math.min(a.y2,b.y2)-Math.max(a.y1,b.y1)),labelBodies=elements.nodes(':visible').filter(n=>!n.isParent()).map(n=>n.boundingBox({includeLabels:false,includeOverlays:false}));
  const labelPositions=(path,z)=>{const out=[];for(const [a,b] of segments(path)){const horizontal=Math.abs(a.y-b.y)<.5,length=Math.abs(horizontal?a.x-b.x:a.y-b.y),extent=(horizontal?z.labelWidth:z.labelHeight)/2+4,fractions=new Set([.5,.35,.65,.2,.8]);for(let offset=extent;offset<=length-extent;offset+=12){fractions.add(offset/length);fractions.add(1-offset/length);}for(const t of fractions){if(length*Math.min(t,1-t)<extent)continue;const x=a.x+(b.x-a.x)*t,y=a.y+(b.y-a.y)*t;out.push({x,y,horizontal,length,t,box:{x1:x-z.labelWidth/2-4,y1:y-z.labelHeight/2-4,x2:x+z.labelWidth/2+4,y2:y+z.labelHeight/2+4}});}}return out;};
  for(const [lane,r] of records.sort((a,b)=>(a.edge.id()===retryEdge?-1:b.edge.id()===retryEdge?1:0)||(Math.abs(a.target.x-a.source.x)+Math.abs(a.target.y-a.source.y))-(Math.abs(b.target.x-b.source.x)+Math.abs(b.target.y-b.source.y))||a.edge.id().localeCompare(b.edge.id())).entries()){
    const sourcePoint=portPoint(r.edge.source(),r.sourceSide,r.sourcePort),targetPoint=portPoint(r.edge.target(),r.targetSide,r.targetPort),start={x:r.source.x+sourcePoint.x-r.edge.source().data('width')/2,y:r.source.y+sourcePoint.y-r.edge.source().data('height')/2},end={x:r.target.x+targetPoint.x-r.edge.target().data('width')/2,y:r.target.y+targetPoint.y-r.edge.target().data('height')/2},sv=vector[r.sourceSide],tv=vector[r.targetSide],sourceStub=stubLength(r.edge.source(),r.sourceSide,sourcePoint,r.sourceStub),targetStub=stubLength(r.edge.target(),r.targetSide,targetPoint,r.targetStub),stub={x:start.x+sv[0]*sourceStub,y:start.y+sv[1]*sourceStub},finish={x:end.x+tv[0]*targetStub,y:end.y+tv[1]*targetStub},routeClearance=48,usedX=occupied.filter(([a,b])=>Math.abs(a.x-b.x)<.5).flatMap(([a])=>[a.x-routeClearance,a.x+routeClearance]),usedY=occupied.filter(([a,b])=>Math.abs(a.y-b.y)<.5).flatMap(([a])=>[a.y-routeClearance,a.y+routeClearance]),channelsX=[(stub.x+finish.x)/2,...xs,...usedX,Math.min(...xs)-24-lane*8,Math.max(...xs)+24+lane*8],channelsY=[(stub.y+finish.y)/2,...ys,...usedY,Math.min(...ys)-24-lane*8,Math.max(...ys)+24+lane*8],candidates=[[start,stub,{x:finish.x,y:stub.y},finish,end],[start,stub,{x:stub.x,y:finish.y},finish,end],...channelsX.map(x=>[start,stub,{x,y:stub.y},{x,y:finish.y},finish,end]),...channelsY.map(y=>[start,stub,{x:stub.x,y},{x:finish.x,y},finish,end])].map(compact),skip=new Set([r.edge.source().id(),r.edge.target().id()]),sourceGroups=new Set(r.edge.source().ancestors().map(n=>n.id())),targetGroups=new Set(r.edge.target().ancestors().map(n=>n.id()));
    const select=paths=>paths.map(path=>{const parts=segments(path);let score=parts.reduce((sum,[a,b])=>sum+Math.abs(a.x-b.x)+Math.abs(a.y-b.y),0)+(path.length-2)*28,blocked=Boolean(r.edge.data('label'))&&!labelPositions(path,r.edge._private.rstyle).some(p=>!labelBodies.some(b=>area(p.box,b)));for(const g of groups){const sourceInside=sourceGroups.has(g.id),targetInside=targetGroups.has(g.id);if(sourceInside&&targetInside)blocked ||= !insideGroup(path,g);else if(sourceInside||targetInside){blocked ||= !leavesOnce(path,g,sourceInside)||parts.some(([a,b])=>hugsGroup(a,b,g));score+=leavesThrough(path,g,sourceInside,facingSide(g,sourceInside?r.target:r.source))?0:100000;}else blocked ||= parts.some(([a,b])=>intersects(a,b,g)||hugsGroup(a,b,g));}for(const [a,b] of parts){blocked ||= obstacles.some(o=>skip.has(o.id)?crossesNode(a,b,cy.getElementById(o.id)):intersects(a,b,o));for(const [c,d] of occupied){blocked ||= parallelGap(a,b,c,d)<14;score+=crossing(a,b,c,d)?180:0;}}return{path,score,blocked};}).filter(choice=>!choice.blocked).sort((a,b)=>a.score-b.score||JSON.stringify(a.path).localeCompare(JSON.stringify(b.path)))[0];
    ...
    if(!scored){if(!retryEdge)return route(elements,r.edge.id());throw new Error(`No collision-free orthogonal route for ${r.edge.id()}.`);}const path=scored.path,bends=path.slice(1,-1),values=segmentValues(bends,r.source,r.target);occupied.push(...segments(path));r.edge.scratch('routePath',path);r.edge.data({curveStyle:bends.length?'segments':'straight',segmentWeights:values.weights,segmentDistances:values.distances,sourceEndpoint:`${start.x-r.source.x}px ${start.y-r.source.y}px`,targetEndpoint:`${end.x-r.target.x}px ${end.y-r.target.y}px`,routeLabelX:0,routeLabelY:0});
  }
  cy.style().update();await new Promise(resolve=>requestAnimationFrame(resolve));
  const placed=[],groupTitles=elements.nodes(':visible').filter(n=>n.isParent()).map(n=>{n.boundingBox({includeLabels:true,includeOverlays:false});return n._private.labelBounds.main;}),labelObstacles=[...labelBodies,...groupTitles];
  const plans=records.filter(r=>r.edge.data('label')).map(r=>{const s=r.edge._private.rscratch,z=r.edge._private.rstyle,path=r.edge.scratch('routePath');if(!Number.isFinite(s.labelX)||!path)return null;const foreignRoutes=records.filter(v=>v!==r).flatMap(v=>segments(v.edge.scratch('routePath'))).map(([a,b])=>({x1:Math.min(a.x,b.x)-2,y1:Math.min(a.y,b.y)-2,x2:Math.max(a.x,b.x)+2,y2:Math.max(a.y,b.y)+2})),candidates=labelPositions(path,z).map(candidate=>({...candidate,staticCollision:[...labelObstacles,...foreignRoutes].reduce((n,v)=>n+area(candidate.box,v),0)}));return{r,s,path,candidates,clean:candidates.filter(v=>v.staticCollision<.01).length};}).filter(Boolean).sort((a,b)=>a.clean-b.clean||a.r.edge.id().localeCompare(b.r.edge.id()));
  for(const {r,s,path,candidates} of plans){let best;for(const candidate of candidates){const collision=candidate.staticCollision+placed.reduce((n,v)=>n+area(candidate.box,v),0),score=collision*1000+(candidate.horizontal?0:60)+Math.abs(candidate.t-.5)*20-candidate.length/100;if(!best||score<best.score)best={...candidate,score};}if(!best){const [a,b]=segments(path).sort((u,v)=>(Math.abs(v[0].x-v[1].x)+Math.abs(v[0].y-v[1].y))-(Math.abs(u[0].x-u[1].x)+Math.abs(u[0].y-u[1].y)))[0];best={x:(a.x+b.x)/2,y:(a.y+b.y)/2};}r.edge.data({routeLabelX:best.x-s.labelX,routeLabelY:best.y-s.labelY});placed.push(best.box);}
  cy.style().update();
}
