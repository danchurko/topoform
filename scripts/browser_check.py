...
AUDIT = r'''() => {
 ...
 const crossesNode=(a,b,n)=>{const c=n.position(),w=n.width()/2,h=n.height()/2,ax=(a.x-c.x)/w,ay=(a.y-c.y)/h,bx=(b.x-c.x)/w,by=(b.y-c.y)/h,dx=bx-ax,dy=by-ay,t=Math.max(0,Math.min(1,-(ax*dx+ay*dy)/(dx*dx+dy*dy||1))),x=ax+t*dx,y=ay+t*dy,shape=n.data('shape'),m=.94;if(shape==='ellipse')return x*x+y*y<m*m;if(shape==='diamond')return Math.abs(x)+Math.abs(y)<m;if(shape==='rhomboid'){if(Math.abs(ay-by)<.001){if(Math.abs(ay)>=m)return false;return Math.max(Math.min(ax,bx),(-2*m+ay)/3)<Math.min(Math.max(ax,bx),(2*m+ay)/3);}const low=Math.max(-m,3*ax-2*m),high=Math.min(m,3*ax+2*m);return Math.max(Math.min(ay,by),low)<Math.min(Math.max(ay,by),high);}return Math.abs(x)<m&&Math.abs(y)<m;};
 const rhomboidProbe={position:()=>({x:0,y:0}),width:()=>220,height:()=>140,data:key=>key==='shape'?'rhomboid':undefined};
 if(!crossesNode({x:88,y:0},{x:88,y:63},rhomboidProbe))errors.push('Rhomboid segment collision regression.');
 ...
 const routeSegments=r=>r.path.slice(1).map((p,i)=>[r.path[i],p]),collinearOverlap=(a,b,c,d)=>{const ah=Math.abs(a.y-b.y)<1,ch=Math.abs(c.y-d.y)<1;if(ah!==ch)return 0;if(ah&&Math.abs(a.y-c.y)<1)return Math.max(0,Math.min(Math.max(a.x,b.x),Math.max(c.x,d.x))-Math.max(Math.min(a.x,b.x),Math.min(c.x,d.x)));if(!ah&&Math.abs(a.x-c.x)<1)return Math.max(0,Math.min(Math.max(a.y,b.y),Math.max(c.y,d.y))-Math.max(Math.min(a.y,b.y),Math.min(c.y,d.y)));return 0;};
 for(let i=0;i<routes.length;i++)for(let j=i+1;j<routes.length;j++)if(routes[i].source!==routes[i].target&&routes[j].source!==routes[j].target)for(const [a,b] of routeSegments(routes[i]))for(const [c,d] of routeSegments(routes[j]))if(collinearOverlap(a,b,c,d)>3)errors.push('Overlapping route channel: '+routes[i].id+' / '+routes[j].id);
