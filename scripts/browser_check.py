...
AUDIT = r'''() => {
 ...
 for(const route of routes)for(const [a,b] of routeSegments(route))for(const g of groupBodies){const horizontal=Math.abs(a.y-b.y)<1,overlap=horizontal?Math.min(Math.max(a.x,b.x),g.x2)-Math.max(Math.min(a.x,b.x),g.x1):Math.min(Math.max(a.y,b.y),g.y2)-Math.max(Math.min(a.y,b.y),g.y1),distance=horizontal?Math.min(Math.abs(a.y-g.y1),Math.abs(a.y-g.y2)):Math.min(Math.abs(a.x-g.x1),Math.abs(a.x-g.x2));if(overlap>3&&distance<18)errors.push('Route hugs group boundary: '+route.id+' / '+g.id);}
